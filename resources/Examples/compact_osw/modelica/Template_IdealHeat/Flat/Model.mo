within Template_IdealHeat.Flat;
model Model "Simple template model for testing OpenStudio-Spawn measures"
  ...
  Buildings.ThermalZones.EnergyPlus_24_2_0.ThermalZone zon
                                                         [n](redeclare package Medium = Medium, zoneName = zoneNames) "Thermal zones" annotation(
    Placement(transformation(extent = {{0, -20}, {40, 20}})));
  Modelica.Blocks.Sources.Constant qIntGai[n,3](each k = 0)
    "Internal heat gains, set to zero because these are modeled in EnergyPlus"                                                         annotation(
