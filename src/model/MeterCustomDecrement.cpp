...
namespace model {
  ...
  namespace detail {
    ...
    MeterCustomDecrement_Impl::MeterCustomDecrement_Impl(const IdfObject& idfObject, Model_Impl* model, bool keepHandle)
      : ModelObject_Impl(idfObject, model, keepHandle) {
      OS_ASSERT(idfObject.iddObject().type() == MeterCustomDecrement::iddObjectType());
    }

    MeterCustomDecrement_Impl::MeterCustomDecrement_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : ModelObject_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == MeterCustomDecrement::iddObjectType());
    }

    MeterCustomDecrement_Impl::MeterCustomDecrement_Impl(const MeterCustomDecrement_Impl& other, Model_Impl* model, bool keepHandle)
      : ModelObject_Impl(other, model, keepHandle) {}
    ...
    bool MeterCustomDecrement_Impl::setFuelType(const FuelType& fuelType) {
      return setString(OS_Meter_CustomDecrementFields::FuelType, fuelType.valueDescription());
    }
    ...
    std::vector<std::pair<std::string, std::string>> MeterCustomDecrement_Impl::keyVarGroups() {
      ...
      for (const auto& group : groups) {
        ...
        boost::optional<std::string> outputVariableorMeterName =
          group.cast<WorkspaceExtensibleGroup>().getString(OS_Meter_CustomDecrementExtensibleFields::OutputVariableorMeterName);
  ...
  std::vector<std::string> MeterCustomDecrement::fuelTypeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Meter_CustomDecrementFields::FuelType);
  }
  ...
  bool MeterCustomDecrement::setFuelType(const FuelType& fuelType) {
  ...
  bool MeterCustomDecrement::setFuelType(const std::string& fuelType) {
    ...
    if (openstudio::istringEqual("Generic", fuelType)) {
      return this->setString(OS_Meter_CustomDecrementFields::FuelType, "Generic");
    }
    try {
