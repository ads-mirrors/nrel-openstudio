...
namespace model {
  ...
  namespace detail {
    ...
    PeopleDefinition_Impl::PeopleDefinition_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : SpaceLoadDefinition_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == PeopleDefinition::iddObjectType());
    }

    PeopleDefinition_Impl::PeopleDefinition_Impl(const PeopleDefinition_Impl& other, Model_Impl* model, bool keepHandle)
      : SpaceLoadDefinition_Impl(other, model, keepHandle) {}
    ...
    bool PeopleDefinition_Impl::pushThermalComfortModelType(const std::string& thermalComfortModelType) {
      return !pushExtensibleGroup(StringVector(1u, thermalComfortModelType)).empty();
    }

    bool PeopleDefinition_Impl::setThermalComfortModelType(int i, const std::string& thermalComfortModelType) {
      ...
      if (i < n) {
        IdfExtensibleGroup eg = getExtensibleGroup(i);
        OS_ASSERT(!eg.empty());
        return eg.setString(OS_People_DefinitionExtensibleFields::ThermalComfortModelType, thermalComfortModelType);
      }
      return false;
    }
    ...
    bool PeopleDefinition_Impl::setNumberOfPeopleCalculationMethod(const std::string& method, double floorArea) {
  ...
  PeopleDefinition::PeopleDefinition(const Model& model) : SpaceLoadDefinition(PeopleDefinition::iddObjectType(), model) {
  ...
  std::vector<std::string> PeopleDefinition::numberofPeopleCalculationMethodValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_People_DefinitionFields::NumberofPeopleCalculationMethod);
  }

  std::vector<std::string> PeopleDefinition::meanRadiantTemperatureCalculationTypeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_People_DefinitionFields::MeanRadiantTemperatureCalculationType);
  }
  ...
  std::vector<std::string> PeopleDefinition::thermalComfortModelTypeValues() {
    ...
    return getIddKeyNames(obj, obj.numFields() + OS_People_DefinitionExtensibleFields::ThermalComfortModelType);
  }
  ...
  /// @cond
  PeopleDefinition::PeopleDefinition(std::shared_ptr<detail::PeopleDefinition_Impl> impl) : SpaceLoadDefinition(std::move(impl)) {}
  /// @endcond
