...
namespace model {
  ...
  namespace detail {
    ...
    MeterCustom_Impl::MeterCustom_Impl(const IdfObject& idfObject, Model_Impl* model, bool keepHandle)
      : ModelObject_Impl(idfObject, model, keepHandle) {
      OS_ASSERT(idfObject.iddObject().type() == MeterCustom::iddObjectType());
    }

    MeterCustom_Impl::MeterCustom_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : ModelObject_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == MeterCustom::iddObjectType());
    }

    MeterCustom_Impl::MeterCustom_Impl(const MeterCustom_Impl& other, Model_Impl* model, bool keepHandle)
      : ModelObject_Impl(other, model, keepHandle) {}
    ...
    std::vector<std::pair<std::string, std::string>> MeterCustom_Impl::keyVarGroups() {
      ...
      for (const auto& group : groups) {
        ...
        boost::optional<std::string> outputVariableorMeterName =
          group.cast<WorkspaceExtensibleGroup>().getString(OS_Meter_CustomExtensibleFields::OutputVariableorMeterName);
  ...
  std::vector<std::string> MeterCustom::fuelTypeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Meter_CustomFields::FuelType);
  }
