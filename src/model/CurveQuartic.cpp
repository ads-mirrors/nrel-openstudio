...
namespace model {
  ...
  namespace detail {
    ...
    CurveQuartic_Impl::CurveQuartic_Impl(const IdfObject& idfObject, Model_Impl* model, bool keepHandle) : Curve_Impl(idfObject, model, keepHandle) {
      OS_ASSERT(idfObject.iddObject().type() == CurveQuartic::iddObjectType());
    }

    CurveQuartic_Impl::CurveQuartic_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : Curve_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == CurveQuartic::iddObjectType());
    }

    CurveQuartic_Impl::CurveQuartic_Impl(const CurveQuartic_Impl& other, Model_Impl* model, bool keepHandle) : Curve_Impl(other, model, keepHandle) {}
  ...
  std::vector<std::string> CurveQuartic::validInputUnitTypeforXValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_QuarticFields::InputUnitTypeforX);
  }

  std::vector<std::string> CurveQuartic::validOutputUnitTypeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_QuarticFields::OutputUnitType);
  }
