...
namespace model {
  ...
  namespace detail {
    ...
    IlluminanceMap_Impl::IlluminanceMap_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : SpaceItem_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == IlluminanceMap::iddObjectType());
    }

    IlluminanceMap_Impl::IlluminanceMap_Impl(const IlluminanceMap_Impl& other, Model_Impl* model, bool keepHandle)
      : SpaceItem_Impl(other, model, keepHandle) {}
    ...
    openstudio::Transformation IlluminanceMap_Impl::transformation() const {
      Vector3d origin(this->originXCoordinate(), this->originYCoordinate(), this->originZCoordinate());
  ...
  IlluminanceMap::IlluminanceMap(const Model& model) : SpaceItem(IlluminanceMap::iddObjectType(), model) {
  ...
  /// @cond
  IlluminanceMap::IlluminanceMap(std::shared_ptr<detail::IlluminanceMap_Impl> impl) : SpaceItem(std::move(impl)) {}
  /// @endcond
