...
namespace model {
  ...
  namespace detail {
    ...
    CurveSigmoid_Impl::CurveSigmoid_Impl(const IdfObject& idfObject, Model_Impl* model, bool keepHandle) : Curve_Impl(idfObject, model, keepHandle) {
      OS_ASSERT(idfObject.iddObject().type() == CurveSigmoid::iddObjectType());
    }

    CurveSigmoid_Impl::CurveSigmoid_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : Curve_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == CurveSigmoid::iddObjectType());
    }

    CurveSigmoid_Impl::CurveSigmoid_Impl(const CurveSigmoid_Impl& other, Model_Impl* model, bool keepHandle) : Curve_Impl(other, model, keepHandle) {}
  ...
  std::vector<std::string> CurveSigmoid::validInputUnitTypeforxValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_SigmoidFields::InputUnitTypeforx);
  }

  std::vector<std::string> CurveSigmoid::validOutputUnitTypeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_SigmoidFields::OutputUnitType);
  }
