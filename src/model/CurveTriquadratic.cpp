...
namespace model {
  ...
  namespace detail {
    ...
    CurveTriquadratic_Impl::CurveTriquadratic_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : Curve_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == CurveTriquadratic::iddObjectType());
    }

    CurveTriquadratic_Impl::CurveTriquadratic_Impl(const CurveTriquadratic_Impl& other, Model_Impl* model, bool keepHandle)
      : Curve_Impl(other, model, keepHandle) {}
    ...
    const std::vector<std::string>& CurveTriquadratic_Impl::outputVariableNames() const {
      static const std::vector<std::string> result{"Performance Curve Output Value", "Performance Curve Input Variable 1 Value",
                                                   "Performance Curve Input Variable 2 Value", "Performance Curve Input Variable 3 Value"};
      return result;
    }
    ...
    bool CurveTriquadratic_Impl::setCoefficient20xPOW2TIMESYPOW2TIMESZPOW2(boost::optional<double> coefficient20xPOW2TIMESYPOW2TIMESZPOW2) {
      ...
      if (coefficient20xPOW2TIMESYPOW2TIMESZPOW2) {
        result =
          setDouble(OS_Curve_TriquadraticFields::Coefficient20x_POW_2_TIMES_y_POW_2_TIMES_z_POW_2, coefficient20xPOW2TIMESYPOW2TIMESZPOW2.get());
      } else {
  ...
  std::vector<std::string> CurveTriquadratic::validInputUnitTypeforXValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_TriquadraticFields::InputUnitTypeforX);
  }

  std::vector<std::string> CurveTriquadratic::validInputUnitTypeforYValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_TriquadraticFields::InputUnitTypeforY);
  }

  std::vector<std::string> CurveTriquadratic::validInputUnitTypeforZValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_TriquadraticFields::InputUnitTypeforZ);
  }

  std::vector<std::string> CurveTriquadratic::validOutputUnitTypeValues() {
    return getIddKeyNames(IddFactory::instance().getObject(iddObjectType()).get(), OS_Curve_TriquadraticFields::OutputUnitType);
  }
