...
namespace model {
  ...
  namespace detail {
    ...
    GeneratorFuelCell_Impl::GeneratorFuelCell_Impl(const IdfObject& idfObject, Model_Impl* model, bool keepHandle)
      : Generator_Impl(idfObject, model, keepHandle) {
      OS_ASSERT(idfObject.iddObject().type() == GeneratorFuelCell::iddObjectType());
    }

    GeneratorFuelCell_Impl::GeneratorFuelCell_Impl(const openstudio::detail::WorkspaceObject_Impl& other, Model_Impl* model, bool keepHandle)
      : Generator_Impl(other, model, keepHandle) {
      OS_ASSERT(other.iddObject().type() == GeneratorFuelCell::iddObjectType());
    }

    GeneratorFuelCell_Impl::GeneratorFuelCell_Impl(const GeneratorFuelCell_Impl& other, Model_Impl* model, bool keepHandle)
      : Generator_Impl(other, model, keepHandle) {}

    const std::vector<std::string>& GeneratorFuelCell_Impl::outputVariableNames() const {
      static const std::vector<std::string> result{
        "Generator Produced AC Electricity Rate", "Generator Produced AC Electricity Energy", "Generator Produced Thermal Rate",
        "Generator Produced Thermal Energy", "Generator Fuel HHV Basis Energy", "Generator Fuel HHV Basis Rate", "Generator Fuel LHV Basis Energy",
        "Generator Fuel Consumption Rate LHV Basis", "FuelCell Heat Loss Rate to Zone", "FuelCell Heat Loss Energy to Zone",
        "FuelCell Convection Heat Loss Rate to Zone", "FuelCell Radiation Heat Loss Rate to Zone",

        // From FuelSupply object
        "Generator Fuel Compressor Electricity Rate", "Generator Fuel Compressor Electricity Energy", "Generator Fuel Compressor Skin Heat Loss Rate"
    ...
    boost::optional<GeneratorFuelCellExhaustGasToWaterHeatExchanger> GeneratorFuelCell_Impl::optionalHeatExchanger() const {
      return getObject<ModelObject>().getModelObjectTarget<GeneratorFuelCellExhaustGasToWaterHeatExchanger>(
  ...
  GeneratorFuelCell::GeneratorFuelCell(const Model& model, const GeneratorFuelCellPowerModule& fCPM, const GeneratorFuelCellAirSupply& fCAS,
                                       ...
                                       const GeneratorFuelCellElectricalStorage& fCES, const GeneratorFuelCellInverter& fCInverter,
    ...
    if (!ok) {
      remove();
      LOG_AND_THROW("Unable to set " << briefDescription() << "'s GeneratorFuelCellPowerModule to " << fCPM.briefDescription() << ".");
    }
    ok = setAirSupply(fCAS);
    if (!ok) {
      remove();
      LOG_AND_THROW("Unable to set " << briefDescription() << "'s GeneratorFuelCellAirSupply to " << fCAS.briefDescription() << ".");
    }
    ok = setFuelSupply(fS);
    if (!ok) {
      remove();
      LOG_AND_THROW("Unable to set " << briefDescription() << "'s GeneratorFuelSupply to " << fS.briefDescription() << ".");
    }
    ok = setWaterSupply(fCWS);
    if (!ok) {
      remove();
      LOG_AND_THROW("Unable to set " << briefDescription() << "'s GeneratorFuelCellWaterSupply to " << fCWS.briefDescription() << ".");
    }
    ok = setAuxiliaryHeater(fCAH);
    if (!ok) {
      remove();
      LOG_AND_THROW("Unable to set " << briefDescription() << "'s GeneratorFuelCellAuxiliaryHeater to " << fCAH.briefDescription() << ".");
    }
    ...
    if (!ok) {
      remove();
      LOG_AND_THROW("Unable to set " << briefDescription() << "'s GeneratorFuelCellElectricalStorage to " << fCES.briefDescription() << ".");
    }
  ...
  /// @cond
  GeneratorFuelCell::GeneratorFuelCell(std::shared_ptr<detail::GeneratorFuelCell_Impl> impl) : Generator(std::move(impl)) {}
  /// @endcond
