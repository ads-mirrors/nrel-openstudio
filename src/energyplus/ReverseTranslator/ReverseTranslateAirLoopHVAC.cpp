...
namespace energyplus {
  ...
  OptionalModelObject ReverseTranslator::translateAirLoopHVAC(const WorkspaceObject& workspaceObject) {
    ...
    if (supplyInletNodeName) {
      supplyInletNode.setName(supplyInletNodeName.get());
    }
    if (supplyOutletNodeName) {
      supplyOutletNode.setName(supplyOutletNodeName.get());
    }
    if (demandInletNodeName) {
      demandInletNode.setName(demandInletNodeName.get());
    }
    if (demandOutletNodeName) {
      demandOutletNode.setName(demandOutletNodeName.get());
    }
              ...
              for (int j = 1; (optionalString = _zoneEquipmentList->getString(j)); j = j + 4) {
                ...
                if (zoneEquipmentName) {
                  ...
                  if (istringEqual(optionalString.get(), "AirTerminal:SingleDuct:ConstantVolume:NoReheat")) {
                    _airTerminal =
                      _workspace.getObjectByTypeAndName(IddObjectType::AirTerminal_SingleDuct_ConstantVolume_NoReheat, zoneEquipmentName.get());
