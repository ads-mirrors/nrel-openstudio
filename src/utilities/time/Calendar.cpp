...
Calendar::Calendar()
  : m_startDate(Date(MonthOfYear::Jan, 1, YearDescription().assumedYear())), m_endDate(Date(MonthOfYear::Dec, 31, YearDescription().assumedYear())) {}

/// constructor with year
Calendar::Calendar(int year) : m_startDate(Date(MonthOfYear::Jan, 1, year)), m_endDate(Date(MonthOfYear::Dec, 31, year)) {}

/// constructor with start and end dates
Calendar::Calendar(const Date& startDate, const Date& endDate) : m_startDate(startDate), m_endDate(endDate) {}
...
void Calendar::standardDaylightSavings() {
  m_daylightSavingsStart = Date::fromNthDayOfMonth(NthDayOfWeekInMonth::second, DayOfWeek::Sunday, MonthOfYear::Mar, m_startDate.year());
  m_daylightSavingsEnd = Date::fromNthDayOfMonth(NthDayOfWeekInMonth::first, DayOfWeek::Sunday, MonthOfYear::Nov, m_startDate.year());
}
...
void Calendar::standardHolidays() {
  ...
  addHoliday(Date::fromNthDayOfMonth(NthDayOfWeekInMonth::third, DayOfWeek::Monday, MonthOfYear::Jan, m_startDate.year()), "MLK Day");
  addHoliday(Date::fromNthDayOfMonth(NthDayOfWeekInMonth::third, DayOfWeek::Monday, MonthOfYear::Feb, m_startDate.year()), "Presidents Day");
  addHoliday(Date::fromNthDayOfMonth(NthDayOfWeekInMonth::fifth, DayOfWeek::Monday, MonthOfYear::May, m_startDate.year()), "Memorial Day");
  addHoliday(Date::fromNthDayOfMonth(NthDayOfWeekInMonth::first, DayOfWeek::Monday, MonthOfYear::Sep, m_startDate.year()), "Labor Day");
  addHoliday(Date::fromNthDayOfMonth(NthDayOfWeekInMonth::second, DayOfWeek::Monday, MonthOfYear::Oct, m_startDate.year()), "Columbus Day");

  // Thanksgiving is kind of weird, should we include Friday afterwards (e.g. office) or not (e.g. retail)
  Date thanksgiving = Date::fromNthDayOfMonth(NthDayOfWeekInMonth::fourth, DayOfWeek::Thursday, MonthOfYear::Nov, m_startDate.year());
