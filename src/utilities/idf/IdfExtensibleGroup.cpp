...
bool IdfExtensibleGroup::empty() const {
  if (!m_impl || (m_index >= m_impl->numFields())) {
    return true;
  }
  return false;
}
...
bool IdfExtensibleGroup::isValid(unsigned fieldIndex) const {
  ...
  if (fieldIndex >= m_impl->iddObject().properties().numExtensible) {
    return false;
  }
...
IdfExtensibleGroup::IdfExtensibleGroup(std::shared_ptr<detail::IdfObject_Impl> impl, unsigned index) : m_impl(impl), m_index(index) {
