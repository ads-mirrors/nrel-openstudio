...
std::vector<std::shared_ptr<ClassDefinitionImpl>> ModelicaFileImpl::getClassDefinitions() {
  std::vector<std::shared_ptr<ClassDefinitionImpl>> result;
  ...
  for (auto* ctx : class_definitions) {
    ...
    if (class_def_ctx->class_specifier() && class_def_ctx->class_specifier()->long_class_specifier()) {
      result.emplace_back(std::make_shared<ClassDefinitionImpl>(self, class_def_ctx));  // NOLINT
    }
  }
...
std::shared_ptr<ClassDefinitionImpl> ModelicaFileImpl::getClassDefinitionByName(const std::string& name) {
  auto allClasses = getClassDefinitions();
  for (auto& classDef : allClasses) {
    if (classDef->longClassSpecifier() == name) {
      return classDef;
    }
  }
...
std::shared_ptr<ClassDefinitionImpl> ModelicaFileImpl::addClassDefinition(const std::string& text) {
  ...
  auto updatedClasses = getClassDefinitions();
  if (updatedClasses.empty()) {
    LOG_AND_THROW("Failed to retrieve class definitions after insertion");
  }
  return updatedClasses.back();
}
...
std::vector<ComponentClauseImpl> ClassDefinitionImpl::componentClauses() {
  std::vector<ComponentClauseImpl> result;
  ...
  const auto component_clause = antlr4::tree::xpath::XPath::findAll(ctx(), std::string(ComponentClauseImpl::xpath), parserPtr);
  ...
  for (auto* c : component_clause) {
    ...
    result.emplace_back(file, component_clause_ctx);  // NOLINT
  }
